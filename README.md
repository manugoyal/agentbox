...
Install Node.js 20.11 or newer, Lima 2.2 or newer, Git, and OpenSSH on the host.
Then install Agentbox from this checkout:
...
```tmux
set -ga update-environment " AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY AWS_SESSION_TOKEN AWS_REGION AWS_DEFAULT_REGION GH_TOKEN BRAINTRUST_API_KEY"
```
...
Back in the host checkout, fetch all branches from the exchange. They appear
under `agentbox/EXCHANGE/` without changing the current branch or working tree:
