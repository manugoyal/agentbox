...
test(
  ...
  async (t) => {
    ...
    const agentbox = (...args) =>
      checked(process.execPath, [resolve("bin/agentbox.js"), "git", ...args]);
    ...
    const published = join(f.root, "published.git");
    git("init", "--bare", "--initial-branch=main", published);
    git("remote", "add", "origin", published);
    git("config", "core.hooksPath", "/dev/null");
    agentbox("publish", name, "guest-work");
    assert.equal(
      git("rev-parse", `refs/remotes/agentbox/${name}/guest-work`),
      git("--git-dir", published, "rev-parse", "refs/heads/guest-work"),
    );
    assert.equal(git("rev-parse", "HEAD"), original);
    assert.equal(
      readFileSync(join(host, "work.txt"), "utf8"),
